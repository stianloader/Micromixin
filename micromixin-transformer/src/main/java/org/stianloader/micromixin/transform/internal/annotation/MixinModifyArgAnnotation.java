...
public class MixinModifyArgAnnotation extends MixinAnnotation<MixinMethodStub> {
    ...
    public void apply(@NotNull ClassNode to, @NotNull HandlerContextHelper hctx, @NotNull MixinStub sourceStub,
        ...
        if (labels.size() < this.expect) {
            this.logger.warn(MixinModifyArgAnnotation.class, "Potentially outdated mixin: {}.{} {} expects {} injection points but only found {}.", sourceStub.sourceNode.name, this.injectSource.name, this.injectSource.desc, this.expect, labels.size());
        }
